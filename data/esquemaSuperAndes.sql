...
ADD CONSTRAINT U_LOGIN UNIQUE (EMAIL,CONTRASENA)
ENABLE;

ALTER TABLE USUARIO
ADD CONSTRAINT FK_U_ID_SUCURSAL
...
ADD CONSTRAINT CK_U_PUNTOS
    CHECK (PUNTOS > 0 OR PUNTOS IS NULL)
ENABLE;
...
(
  ...
  PRECIO_PROMOCION NUMBER DEFAULT NULL,
  TIPO VARCHAR2(24 BYTE) DEFAULT NULL, 
...
ADD CONSTRAINT CK_PROM_VENTAS_ACTUALES
    CHECK (VENTAS_ACTUALES>=0 OR VENTAS_ACTUALES IS NULL)
ADD CONSTRAINT CK_PROM_PRECIO_PAQUETE
    CHECK ((PRECIO_PROMOCION>0) OR PRECIO_PROMOCION IS NULL)  
...
(
    ID_PROMOCION NUMBER,
    CODIGO_PRODUCTO VARCHAR2(24 BYTE),
    CONSTRAINT PROMOCION_PRODUCTO_PK PRIMARY KEY (ID_PROMOCION,CODIGO_PRODUCTO)
);
ALTER TABLE PROMOCION_PRODUCTO
ADD CONSTRAINT FK_PROM_CODIGO_BARRAS
    FOREIGN KEY (CODIGO_PRODUCTO)
    REFERENCES PRODUCTO(CODIGO_BARRAS)
ENABLE;
ALTER TABLE PROMOCION_PRODUCTO
ADD CONSTRAINT FK_PROM_PROMOCION
...
-- Creacion de la tabla CARRITO y especificacion de sus restricciones
CREATE TABLE CARRITO
   (ID_CARRITO NUMBER,
    ID_USUARIO NUMBER,
    ID_SUCURSAL NUMBER,

	CONSTRAINT CARRITO PRIMARY KEY (ID_CARRITO,ID_SUCURSAL));
    
ALTER TABLE CARRITO
ADD CONSTRAINT FK_VP_ID_USUARIO
...
CREATE TABLE PRODUCTO_CARRITO
    ...
    CONSTRAINT PRODUCTO_CARRITO PRIMARY KEY (ID_CARRITO,ID_SUCURSAL,ID_PRODUCTO)
    );
    
ALTER TABLE PRODUCTO_CARRITO
ADD CONSTRAINT FK_PC_ID_CARRITO
    FOREIGN KEY (ID_CARRITO,ID_SUCURSAL)
    REFERENCES CARRITO(ID_CARRITO,ID_SUCURSAL)
ENABLE;   
   

ALTER TABLE PRODUCTO_CARRITO
ADD CONSTRAINT FK_PC_ID_PRODUCTO
    FOREIGN KEY (ID_PRODUCTO)
    REFERENCES PRODUCTO(CODIGO_BARRAS)
ENABLE;

ALTER TABLE PRODUCTO_CARRITO
ADD CONSTRAINT CK_PC_CANTIDAD
    CHECK (CANTIDAD > 0)
...
ADD CONSTRAINT CK_ESTADO
    CHECK (ESTADO IN('En carrito','Abandonado','Devuelto','Comprado'))
ENABLE;
